...
- [-] 4. TIM（Three Instruction Machine）
    ...
    - [x] 4.5 Mark 4: 更新
        ...
        - [x] 4.5.6 間接参照鎖の除去
        - [x] 4.5.7 部分適用の更新
    - [x] 4.6 Mark 5: 構造を持つデータ
        - [x] 4.6.1 汎用的アプローチ
        - [x] 4.6.2 構造を持つデータに対する遷移規則とコンパイル図式
        - [x] 4.6.3 トライアウト
        - [x] 4.6.4 リストの印字
        - [x] 4.6.5 構造をもつデータ構造の直接使用
    - [x] 4.7 Mark 6: CAFとコードストア
        - [x] 4.7.1 CAFの実装
        - [x] 4.7.2 より正確なコードストアのモデル
    - [x] 4.8 まとめ        
- [x] 5. 並列G-machine
    - [x] 5.1 導入
        - [x] 5.1.1 並列関数プログラミング
        - [x] 5.1.2 並列グラフ簡約
    - [x] 5.2 Mark 1: 最小並列G-Machine
        - [x] 5.2.1 データ型定義
        - [x] 5.2.2 評価器
        - [x] 5.2.3 プログラムのコンパイル
        - [x] 5.2.4 結果の印字
    - [x] 5.3 Mark 2: 評価-停止モデル
        - [x] 5.3.1 ノードのデータ型
        - [x] 5.3.2 命令セット
    - [x] 5.4 Mark 3: 現実的な並列G-Machine
        - [x] 5.4.1 スケジューリング方針
        - [x] 5.4.2 保守的並列と投機的並列
    - [x] 5.5 Mark 4: よりよいブロッキング処理
        - [x] 5.5.1 データ構造
        - [x] 5.5.2 ロックとアンロック
    - [x] 5.6 まとめ
- [ ] 6. λ 持ち上げ
    - [x] 6.1 導入
    - [x] 6.2 `Expr`データ型の改良
    - [x] 6.3 Mark 1: 単純な λ 持ち上げ器
        - [x] 6.3.1 単純な λ 持ち上げ器の実装
        - [x] 6.3.2 自由変数
        - [x] 6.3.3 スーパーコンビネータの生成
        - [x] 6.3.4 変数を非重複にする
        - [x] 6.3.5 スーパーコンビネータの収集
    - [x] 6.4 Mark 2: 単純な λ 持ち上げ器の改良
        - [x] 6.4.1 単純な拡張
        - [x] 6.4.2 冗長なスーパーコンビネータの除去
        - [x] 6.4.3 冗長な局所定義の除去
    - [x] 6.5 Mark 3: Johnsson流 λ 持ち上げ
        - [x] 6.5.1 実装
        - [x] 6.5.2 関数中の自由変数の抽象化
        - [x] 6.5.3 難しい点
    - [ ] 6.6 Mark 4: 独立した完全怠惰パス
        - [x] 6.6.1 完全怠惰の復習
        - [x] 6.6.2 `let(rec)`存在下の完全怠惰 λ 持ち上げ
        - [x] 6.6.3 λ 持ち上げのない完全怠惰
        - [x] 6.6.4 完全怠惰 λ 持ち上げ器
        - [x] 6.6.5 λ の分離
        - [ ] 6.6.6 レベル番号の追加
