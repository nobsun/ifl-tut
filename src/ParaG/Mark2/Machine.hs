...
steps state = case concatMap (map toLower) $ take 1 state.ctrl of
    ...
    where
        state' = state { pgmGlobal = global
                       , pgmLocals = locals
                       }
        (global, locals) = mapAccumL step global' locals'
        locals' = map tick (state.pgmLocals ++ newtasks)
        (global', newtasks) = mapAccumL phi (state.pgmGlobal { sparks = [] }) 
                            $ state.pgmGlobal.sparks
        phi g a = (g { maxtid = tid}, makeTask tid a)
                    where
                        tid = succ g.maxtid
