...
data Node
    ...
    | NLAp Addr Addr GmTaskId PgmPendingList      -- Locked applications
    | NLGlobal Int GmCode GmTaskId PgmPendingList -- Locked globals
    deriving (Eq, Show)

type GmTaskId = Int
type PgmPendingList = [PgmLocalState]

emptyPendingList :: PgmPendingList -> GmState -> GmState
emptyPendingList tasks (global, local)
    = (global { sparks = tasks ++ global.sparks }, local)
